...
local function onexplode(inst)
    ...
    if not inst:IsOnValidGround() then
        ...
        if scorch ~= nil then
			...
			scorch:AddComponent("rp_autodelete")
			scorch.components.rp_autodelete:SetPerishTime(20)
			scorch.components.rp_autodelete:StartPerishing()
        end
        local launched = {}
        ...
        for i, v in ipairs(ents) do
            ...
            if v:IsValid() and not v:IsInLimbo() then
                ...
                elseif v.components.combat ~= nil and v.prefab ~= "rocky" then

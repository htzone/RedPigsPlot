	...
	--产生入侵
	local function makeInvaders(world, player, monster_str)
		...
		for k = 1, invaders_num do
			...
			if monster then
				...
				monster.components.rp_invasive:Make(INVASIVE_MOB)
			end			
		end
	end
	...
	local function updateWorld(world)
		...
		if world.canInvade then
			if TheWorld.state.cycles % 1 == 0 then
				local invasive_monster = invade_table[math.random(#invade_table)]
				if not table.contains(world.cant_invade_table, invasive_monster) then
					world:DoTaskInTime(2,function()
						local allow_players = GetSpawnPlayerList()
						if next(allow_players) ~= nil then
							for k=1, #allow_players do
								makeInvaders(world, allow_players[k], invasive_monster)--产生入侵
							end
						end
					end)
				end
	...
	local RP_Monster_Invade = Class(function(self, inst)
		self.inst = inst
		self.inst.canInvade = true
		self.inst.cant_invade_table = {}
		self.inst:ListenForEvent("ms_cyclecomplete", function() updateWorld(self.inst) end)
		
	end)
	
	function RP_Monster_Invade:SetIfCanInvade(canInvade)
		self.inst.canInvade = canInvade
	end
	
	function RP_Monster_Invade:AddCantInvadeMob(monster)
		if monster ~= nil then
			table.insert(self.inst.cant_invade_table, monster)
		end
	end
	
	function RP_Monster_Invade:RemoveCantInvadeMob(monster)
		if monster ~= nil then
			RemoveByValue(self.inst.cant_invade_table, monster)
		end
	end
	
	function RP_Monster_Invade:OnSave()
		return
		{
			canInvade = self.inst.canInvade,
			cant_invade_table = self.inst.cant_invade_table
		}
	end
	
	function RP_Monster_Invade:OnLoad(data)
		if data ~= nil then
			self.inst.canInvade = data.canInvade
			self.inst.cant_invade_table = data.cant_invade_table
		end
	end
	
	return RP_Monster_Invade
...
--[[小野猪]]
--------------------------------------------------------------------------
local brain = require "brains/rp_point_piglittlebrotherkingbrain"
...
local function MakeWeapon(inst)
    if inst.components.inventory ~= nil then
        ...
        weapon:AddComponent("weapon")
        weapon.components.weapon:SetDamage(30)
        ...
        weapon.components.weapon:SetProjectile("poop")
        weapon:AddComponent("inventoryitem")
        weapon.persists = false
        weapon.components.inventoryitem:SetOnDroppedFn(weapon.Remove)
        weapon:AddComponent("equippable")
...
--self.inst:AddTag("BigLittleBrother")
...
--血多厚
self.inst.components.health:SetMaxHealth(LITTLE_PIGBROTHER_HEALTH)
...
self.inst.components.combat:SetRange(12, 14)

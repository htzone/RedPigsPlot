...
local function onattackfn(inst, data)
	...
	and not inst.sg:HasStateTag("busy") then	
		...
		--if not (inst.angry or inst.components.timer:TimerExists("angry")) then
		--	inst.components.timer:StartTimer("angry", 4)
		--end
			
		--if inst.angry then
		--	inst.sg:GoToState("angry")
		--else
...
{	
	...
	State{
        ...
        events=
        {
            EventHandler("animover", function(inst) 
				if not inst.components.timer:TimerExists("angry") then
					inst.components.timer:StartTimer("angry", 4)
				end
	...
	State{
        ...
        timeline=
        {
            ...
            TimeEvent(23*FRAMES, function(inst) inst.components.combat:DoAttack() inst.sg:RemoveStateTag("attack") inst.sg:RemoveStateTag("busy") end),
        },
